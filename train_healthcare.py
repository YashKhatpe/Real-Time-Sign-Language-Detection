...
def run(configs,
    ...
    for i, batch in enumerate(dataloader):
        ...
        inputs, labels, *extra = batch  # Unpacking with support for extra elements

        print(f"Element 0 (Inputs): Type: {type(inputs)}, Shape: {inputs.shape}")  
        print(f"Element 1 (Labels): Type: {type(labels)}, Shape: {labels.shape}")  

        # Checking if it's multi-label or single-label
        unique_labels = labels.unique()
        print(f"Unique label values: {unique_labels}")

        if labels.shape[1] > 1:  
            print("Labels are multi-label (more than one possible class per sample).")
        else:
            print("Labels are single-label (only one class per sample).")
