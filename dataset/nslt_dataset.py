...
def load_rgb_frames_from_video(vid_root, vid, start, num, resize=(256, 256)):
    # Search for the video inside train, test, or val subfolders
    possible_paths = glob.glob(os.path.join(vid_root, "*", vid + ".mp4"))

    if not possible_paths:
        print(f"Error: Video {vid}.mp4 not found in train/test/val subfolders.")
        return []  # Return empty list to avoid crashing

    video_path = possible_paths[0].replace('\\', '/')  # Use the first match
...
def make_dataset(split_file, split, root, mode, num_classes):
    ...
    for vid in data.keys():
        ...
        vid_root = root['word']
        src = 0
        subset_folder = data[vid]['subset']  # Get the folder name (train/test/val)
        video_path = os.path.join(vid_root, subset_folder, vid + '.mp4').replace('\\', '/')

        # video_path = os.path.join(vid_root, vid + '.mp4')
        # video_path = os.path.join(vid_root, vid + '.mp4').replace('\\', '/')
